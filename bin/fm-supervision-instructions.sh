...
case "$HARNESS" in
  claude|codex|opencode|pi|grok|cursor|omp|codebuddy) SNIPPET="$DOC_DIR/$HARNESS.md" ;;
...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      ...
      ;;
    codebuddy)
      printf '%s%s\n' "$prefix" 'watcher supervision needs Stop-owned automatic recovery; inspect the Stop registration in the tracked .codebuddy/settings.json and the watcher startup status before ending the turn.'
      ;;
    codex)
...
ordinary_wake_line() {
  case "$HARNESS" in
    claude)
      ...
      ;;
    codebuddy)
      printf '%s\n' '- Ordinary wake: the Stop-owned auto-arm (bin/fm-claude-stop-autoarm.sh, registered in the tracked .codebuddy/settings.json) already owns watcher continuity; drain and handle the wake, and do not arm another cycle yourself.'
      ;;
    codex)
