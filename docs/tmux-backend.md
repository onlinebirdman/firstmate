...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, Muse, Rovo, AGY, omp, and CodeBuddy process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
CodeBuddy is anchored to the exact `codebuddy` and `cbc` identities for the same reason, so a bare fragment of either name remains ambiguous.
