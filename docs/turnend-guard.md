...
- CodeBuddy registers the same two `Stop` hooks in its tracked `.codebuddy/settings.json`, both anchored through `CLAUDE_PROJECT_DIR`: `bin/fm-turnend-guard.sh --codebuddy`, and `bin/fm-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`; `--codebuddy` is an alias for the `--claude` autoarm-cooperation mode, so a CodeBuddy primary shares Claude's supervision model.
