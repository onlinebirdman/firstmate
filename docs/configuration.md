...
codebuddy is verified for primary, crewmate, scout, and secondmate launches: it is a Claude-compatible harness whose project settings live in a tracked `.codebuddy/settings.json`, so a codebuddy primary runs the same Stop-hook `asyncRewake` auto-arm and turn-end guard as Claude Code; [`docs/supervision-protocols/codebuddy.md`](supervision-protocols/codebuddy.md) owns that protocol and [its adapter reference](../.agents/skills/harness-adapters/references/harness/codebuddy.md) owns the remaining boundaries.
Its secondmate boundary opened on 2026-09-15, once that primary supervision path was live-verified; its worker busy-state source, interactive composer, and interrupt behavior remain unverified for codebuddy crewmates.
...
CodeBuddy is a quota surface `quota-axi` does not model: its allowance is account plan/bonus credits from the optional `opencli` account library, read by [`bin/fm-codebuddy-usage.sh`](../bin/fm-codebuddy-usage.sh).
A quota-aware codebuddy dispatch joins that script's schemaVersion 5 fragment - `fm-codebuddy-usage.sh quota` - to the `quota-axi` snapshot with `fm_quota_json_compose`, which [`bin/fm-quota-choose.sh`](../bin/fm-quota-choose.sh) accepts as `--companion <file>`.
Because the fragment is derived from the account library's own numbers, an absent `opencli` or an unmeasurable account is disclosed uncertainty rather than a fabricated figure, so `opencli` stays optional and never blocks bootstrap.
